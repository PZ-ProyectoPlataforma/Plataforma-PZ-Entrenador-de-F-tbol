...
const TACTICAL_ELEMENTS = [
  { type: "jugadorA", label: "Jugador (peto A)", isPlayer: true },
  { type: "jugadorB", label: "Jugador (peto B)", isPlayer: true },
  { type: "jugadorC", label: "Jugador (peto C)", isPlayer: true },
  { type: "portero", label: "Portero", isPlayer: true, letter: "G" },
  { type: "balon", label: "Balón", isEmoji: true, emoji: "⚽" },
  { type: "porteria", label: "Portería", isResizable: true },
  { type: "cono", label: "Cono" },
  { type: "estaca", label: "Estaca / pica" },
  { type: "valla", label: "Valla de coordinación" },
  { type: "escalera", label: "Escalera de coordinación" },
  { type: "aro", label: "Aro" },
  { type: "barrera", label: "Barrera (maniquí)" },
  { type: "texto", label: "Texto libre", needsText: true },
];
const LINE_TYPES = [
...
const FIELD_TYPES = [
  ...
  ["zonaRedonda", "Zona redonda"], ["zonaEsquina", "Zona de la esquina"],
];
...
function cornerArc(cx, cy, qx, qy) { return `<path d="M${cx},${cy - qy} A${Math.abs(qy)},${Math.abs(qy)} 0 0 ${qx > 0 ? 1 : 0} ${cx + qx},${cy}" fill="none" stroke="${PITCH_LINE}" stroke-width="1.5"/>`; }
...
function renderFieldBackground(field) {
  ...
  if (field === "zonaEsquina") {
    return `${green}
      <line x1="10" y1="10" x2="${W - 10}" y2="10" stroke="${PITCH_LINE}" stroke-width="2"/>
      <line x1="10" y1="10" x2="10" y2="${H - 10}" stroke="${PITCH_LINE}" stroke-width="2"/>
      <path d="M10,45 A35,35 0 0 1 45,10" fill="none" stroke="${PITCH_LINE}" stroke-width="1.5"/>
      <line x1="230" y1="10" x2="230" y2="220" stroke="#F4B400" stroke-width="2" stroke-dasharray="9,6"/>
      <line x1="10" y1="220" x2="230" y2="220" stroke="#F4B400" stroke-width="2" stroke-dasharray="9,6"/>`;
  }

  // futbol11 / area / medioCampo / dobleArea: comparten el mismo dibujo de área+portería a la izquierda
  ...
  svg += cornerArc(10, 10, 22, 22);
  svg += cornerArc(10, H - 10, 22, -22);

  if (field === "area") {
    svg += `<path d="M${8 + boxBig.w},${cy - 55} A55,55 0 0 1 ${8 + boxBig.w},${cy + 55}" fill="none" stroke="${PITCH_LINE}" stroke-width="1.8"/>`;
    return svg;
  }
  if (field === "medioCampo" || field === "dobleArea") {
    const midX = 320;
    svg += `<line x1="${midX}" y1="8" x2="${midX}" y2="${H - 8}" stroke="${PITCH_LINE}" stroke-width="1.8"/>`;
    svg += `<circle cx="${midX}" cy="${cy}" r="65" fill="none" stroke="${PITCH_LINE}" stroke-width="1.8"/>`;
    svg += `<circle cx="${midX}" cy="${cy}" r="2" fill="${PITCH_LINE}"/>`;
    if (field === "dobleArea") {
      svg += `<line x1="${midX}" y1="${cy - boxBig.h / 2}" x2="${W - 60}" y2="${cy - boxBig.h / 2}" stroke="#F4B400" stroke-width="1.8" stroke-dasharray="9,6"/>`;
      svg += `<line x1="${midX}" y1="${cy + boxBig.h / 2}" x2="${W - 60}" y2="${cy + boxBig.h / 2}" stroke="#F4B400" stroke-width="1.8" stroke-dasharray="9,6"/>`;
      svg += `<line x1="${W - 60}" y1="${cy - boxBig.h / 2}" x2="${W - 60}" y2="${cy + boxBig.h / 2}" stroke="#F4B400" stroke-width="1.8" stroke-dasharray="9,6"/>`;
      svg += goalSVG(W - 60, cy, 20, 40, "right");
    }
    return svg;
  }
  // futbol11 completo
  ...
  svg += cornerArc(W - 10, 10, -22, 22);
  svg += cornerArc(W - 10, H - 10, -22, -22);
...
function elementIconSVG(type) {
  switch (type) {
    ...
    case "cono": return `<ellipse cx="0" cy="8" rx="7" ry="2.2" fill="#D97706"/><path d="M-6,8 L-1.8,-10 L1.8,-10 L6,8 Z" fill="#F4B400" stroke="#B45309" stroke-width="0.8"/><rect x="-3.4" y="1" width="6.8" height="2" fill="#B45309"/>`;
    ...
    case "valla": return `<line x1="-8" y1="10" x2="-8" y2="-7" stroke="#0B1F3A" stroke-width="2"/><line x1="8" y1="10" x2="8" y2="-7" stroke="#0B1F3A" stroke-width="2"/><line x1="-8" y1="-7" x2="8" y2="-7" stroke="#E63946" stroke-width="2.6"/><line x1="-8" y1="0" x2="8" y2="0" stroke="#E63946" stroke-width="2.2"/>`;
...
function buildDiagramSVG(diagram) {
  ...
  const lines = (diagram.lines || []).map((l) => {
    ...
    else if (l.type === "salto") { const mx = (x1 + x2) / 2, my = (y1 + y2) / 2 - 30; d = `M${x1},${y1} Q${mx},${my} ${x2},${y2}`; }
    else {
      const dx = x2 - x1, dy = y2 - y1, dist = Math.sqrt(dx * dx + dy * dy) || 1;
      const nx = -dy / dist, ny = dx / dist, segs = 5;
      let path = `M${x1},${y1}`;
      for (let i = 1; i <= segs; i++) { const t = i / segs, amp = (i % 2 === 0 ? 1 : -1) * 8; path += ` L${x1 + dx * t + nx * amp},${y1 + dy * t + ny * amp}`; }
      d = path;
    }
  ...
  const elements = (diagram.elements || []).map((el) => {
    ...
    if (def.isPlayer) {
      ...
      return `<g><circle cx="${x}" cy="${y}" r="13" fill="${PLAYER_COLORS[el.type]}" stroke="#fff" stroke-width="2"/>${label ? `<text x="${x}" y="${y + 4.5}" font-size="12" font-weight="800" fill="#fff" text-anchor="middle">${esc(label)}</text>` : ""}</g>`;
    }
    if (def.isEmoji) {
      const size = 24 * ((el.size || 45) / 45);
      return `<text x="${x}" y="${y + size / 3}" font-size="${size}" text-anchor="middle">${def.emoji}</text>`;
    }
    if (el.type === "porteria") {
      const scale = (el.size || 45) / 45;
      return `<g transform="translate(${x},${y})">${goalSVG(0, 0, 22 * scale, 30 * scale, "right")}</g>`;
    }
    if (el.type === "texto") {
      const fs = 15;
      const w = Math.max(24, (el.text || "").length * fs * 0.6 + 12);
      return `<g><rect x="${x - w / 2}" y="${y - 13}" width="${w}" height="24" rx="6" fill="#fff" stroke="#0B1F3A" stroke-width="1.4"/><text x="${x}" y="${y + 5}" font-size="${fs}" font-weight="800" fill="#0B1F3A" text-anchor="middle">${esc(el.text)}</text></g>`;
    }
    return `<g transform="translate(${x},${y})">${elementIconSVG(el.type)}</g>`;
  }).join("");
...
function TacticalEditorModal({ initial, onClose, onSave }) {
  ...
  const onCanvasClick = (e) => {
    ...
    if (!pendingPoint) setPendingPoint(pt);
      ...
      setDiagram((p) => ({ ...p, lines: [...p.lines, { id: uid(), type: lineType, color: lineColor, x1: pendingPoint.x, y1: pendingPoint.y, x2: pt.x, y2: pt.y }] }));
  ...
  return (
    <Modal title="Tarea gráfica" onClose={onClose} wide>
      <div className="flex flex-col gap-3">
        ...
        {pendingPoint && lineMode && <div className="text-xs font-bold" style={{ color: T.red }}>Toca el punto final de la línea…</div>}
