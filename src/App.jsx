...
/* ============================================================
   ...
   ============================================================ */
const PLAYER_COLORS = { jugadorA: "#1E3A8A", jugadorB: "#E63946", jugadorC: "#2A9D5C", portero: "#F4B400" };
...
const LINE_COLORS = [["Azul (frente)", "#1E3A8A"], ["Amarillo (espaldas)", "#F4B400"], ["Verde (salto)", "#2A9D5C"], ["Rojo (remate/pase)", "#E63946"]];
const FIELD_TYPES = [
...
function renderFieldBackground(field) {
  ...
  if (field === "area" || field === "medioCampo") {
    ...
    if (field === "area") {
      svg += `<path d="M${cx - 55},${topY + boxBig.h} A55,55 0 0 1 ${cx + 55},${topY + boxBig.h}" fill="none" stroke="${PITCH_LINE}" stroke-width="1.8"/>`;
      return svg;
    }
    const midY = topY + 260;
...
function buildDiagramSVG(diagram) {
  ...
  const lines = (diagram.lines || []).map((l) => {
    ...
    return `<path d="${d}" fill="none" stroke="${l.color}" stroke-width="1.6" ${dash} marker-end="url(#arrow-${l.color.replace("#", "")})"/>`;
  }).join("");
  ...
  const elements = (diagram.elements || []).map((el) => {
    ...
    if (def.isPlayer) {
      ...
      const r = 13 * scale;
      return `<g><circle cx="${x}" cy="${y}" r="${r}" fill="${PLAYER_COLORS[el.type]}" stroke="#fff" stroke-width="2"/>${label ? `<text x="${x}" y="${y + r * 0.35}" font-size="${11 * scale}" font-weight="800" fill="#fff" text-anchor="middle">${esc(label)}</text>` : ""}</g>`;
    }
    if (def.isZona) {
      const size = 60 * scale;
  ...
  const onCanvasClick = (e) => {
    ...
    if (lineType === "salto") {
      ...
      if (!pendingPoint) setPendingPoint({ step: 1, x1: pt.x, y1: pt.y });
        ...
        setDiagram((p) => ({ ...p, lines: [...p.lines, { id: uid(), type: "salto", color: lineColor, x1: pendingPoint.x1, y1: pendingPoint.y1, x2: pt.x, y2: pt.y, cx: pendingPoint.cx, cy: pendingPoint.cy }] }));
        setPendingPoint(null);
      }
    } else {
      if (!pendingPoint) setPendingPoint({ step: 1, x1: pt.x, y1: pt.y });
      else {
        setDiagram((p) => ({ ...p, lines: [...p.lines, { id: uid(), type: lineType, color: lineColor, x1: pendingPoint.x1, y1: pendingPoint.y1, x2: pt.x, y2: pt.y }] }));
        setPendingPoint(null);
      }
    }
  };
          ...
          <div className="p-2.5 rounded-lg" style={{ background: "#FEF3C7", border: `1.5px solid ${T.yellow}` }}>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {LINE_COLORS.map(([label, c]) => (
                <button key={c} onClick={() => setLineColor(c)} className="w-7 h-7 rounded-full border-2" style={{ background: c, borderColor: lineColor === c ? T.navy : "#fff" }} title={label} />
              ))}
            </div>
          </div>
        )}
