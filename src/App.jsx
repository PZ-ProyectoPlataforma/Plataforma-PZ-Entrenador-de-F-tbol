...
const TACTICAL_ELEMENTS = [
  { type: "jugadorA", label: "Jugador (peto A)", icon: "●", isPlayer: true },
  { type: "jugadorB", label: "Jugador (peto B)", icon: "●", isPlayer: true },
  { type: "jugadorC", label: "Jugador (peto C)", icon: "●", isPlayer: true },
  { type: "portero", label: "Portero", icon: "●", isPlayer: true, letter: "G" },
  { type: "balon", label: "Balón" },
  { type: "porteria", label: "Portería" },
  { type: "minipuerta", label: "Minipuerta" },
  ...
  { type: "area", label: "Área de trabajo" },
  { type: "letra", label: "Letra (A, B, C…)", needsText: true },
  { type: "texto", label: "Texto libre", needsText: true },
  { type: "zonaRect", label: "Zona rectangular", isZona: true, shape: "rect" },
  { type: "zonaCirc", label: "Zona circular", isZona: true, shape: "circle" },
  { type: "zonaTri", label: "Zona triangular", isZona: true, shape: "triangle" },
];
const LINE_TYPES = [
...
const FIELD_TYPES = [["futbol11", "Fútbol 11"], ["futbol8", "Fútbol 8"], ["medio", "Medio campo"], ["libre", "Espacio libre"]];
const DIAGRAM_W = 600, DIAGRAM_H = 400;

/* Íconos vectoriales propios (no emoji), para verse igual en cualquier dispositivo */
function elementIconSVG(type) {
  switch (type) {
    case "balon": return `<circle r="8" fill="#fff" stroke="#1B2733" stroke-width="1.2"/><path d="M0,-8 L4.7,-2.5 L2.9,4.5 L-2.9,4.5 L-4.7,-2.5 Z" fill="#1B2733"/>`;
    case "porteria": return `<rect x="-17" y="-15" width="34" height="15" fill="none" stroke="#fff" stroke-width="2"/><path d="M-17,-15 L0,0 M0,-15 L-8,0 M8,-15 L17,0 M17,-15 L0,0" stroke="#fff" stroke-width="0.6" opacity="0.7"/>`;
    case "minipuerta": return `<rect x="-9" y="-8" width="18" height="8" fill="none" stroke="#fff" stroke-width="1.8"/>`;
    ...
    case "estaca": return `<ellipse cx="0" cy="10" rx="4" ry="1.3" fill="#1F7A44"/><line x1="0" y1="10" x2="0" y2="-11" stroke="#2A9D5C" stroke-width="2.2"/><circle cx="0" cy="-11" r="2.6" fill="#2A9D5C"/>`;
    ...
    case "escalera": return `<line x1="-6" y1="12" x2="-6" y2="-12" stroke="#8B5E34" stroke-width="2"/><line x1="6" y1="12" x2="6" y2="-12" stroke="#8B5E34" stroke-width="2"/>` + [-9, -4.5, 0, 4.5, 9].map((yy) => `<line x1="-6" y1="${yy}" x2="6" y2="${yy}" stroke="#B08968" stroke-width="1.8"/>`).join("");
...
function buildDiagramSVG(diagram) {
  ...
  const field = diagram.field || "futbol11";
  let bg;
  if (field === "libre") {
    bg = `<rect width="${DIAGRAM_W}" height="${DIAGRAM_H}" fill="#fff"/><rect width="${DIAGRAM_W}" height="${DIAGRAM_H}" fill="none" stroke="#E2E8F0" stroke-width="1"/>`;
  } else {
    const boxW = field === "futbol8" ? 46 : 60, boxH = field === "futbol8" ? 90 : 120;
    bg = `<rect width="${DIAGRAM_W}" height="${DIAGRAM_H}" fill="#2A9D5C"/>
      <rect x="8" y="8" width="${DIAGRAM_W - 16}" height="${DIAGRAM_H - 16}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>
      ${field === "medio" ? "" : `<line x1="${DIAGRAM_W / 2}" y1="8" x2="${DIAGRAM_W / 2}" y2="${DIAGRAM_H - 8}" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>`}
      <circle cx="${DIAGRAM_W / 2}" cy="${DIAGRAM_H / 2}" r="${field === "medio" ? 45 : 40}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>
      ${field === "medio"
        ? `<rect x="${DIAGRAM_W / 2 - boxH / 2}" y="${DIAGRAM_H - 8 - boxW}" width="${boxH}" height="${boxW}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>`
        : `<rect x="8" y="${DIAGRAM_H / 2 - boxH / 2}" width="${boxW}" height="${boxH}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>
           <rect x="${DIAGRAM_W - 8 - boxW}" y="${DIAGRAM_H / 2 - boxH / 2}" width="${boxW}" height="${boxH}" fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.5"/>`}`;
  }
  ...
  const elements = (diagram.elements || []).map((el) => {
    ...
    if (def.isZona) {
      const size = el.size || 45;
      const style = `fill="rgba(244,180,0,0.22)" stroke="#F4B400" stroke-width="1.5" stroke-dasharray="5,4"`;
      if (def.shape === "circle") return `<circle cx="${x}" cy="${y}" r="${size / 2}" ${style}/>`;
      if (def.shape === "triangle") { const r = size / 2; return `<polygon points="${x},${y - r} ${x + r},${y + r * 0.8} ${x - r},${y + r * 0.8}" ${style}/>`; }
      return `<rect x="${x - size / 2}" y="${y - size / 3}" width="${size}" height="${size * 0.66}" ${style}/>`;
    }
    if (el.type === "area") return `<rect x="${x - 50}" y="${y - 32}" width="100" height="64" fill="none" stroke="#fff" stroke-width="1.5" stroke-dasharray="4,3"/>`;
    if (el.type === "letra" || el.type === "texto") {
      const fs = el.type === "letra" ? 15 : 11;
      const w = Math.max(22, (el.text || "").length * fs * 0.62 + 10);
      return `<g><rect x="${x - w / 2}" y="${y - 12}" width="${w}" height="22" rx="5" fill="#fff" stroke="#0B1F3A" stroke-width="1.4"/><text x="${x}" y="${y + 4}" font-size="${fs}" font-weight="800" fill="#0B1F3A" text-anchor="middle">${esc(el.text)}</text></g>`;
    }
...
function TacticalEditorModal({ initial, onClose, onSave }) {
  ...
  const [textPromptType, setTextPromptType] = useState(null);
  ...
  const handleToolTap = (el) => {
    if (el.needsText) { setTextPromptType(el.type); return; }
    addElement(el.type);
  };
  ...
  const onCanvasPointerDown = (e) => {
    ...
    setPreviewLine({ x1: pt.x, y1: pt.y, x2: pt.x, y2: pt.y });
  };
  const onPointerMove = (e) => {
    if (dragId.current) {
      ...
      const pt = toPercent(e.clientX, e.clientY);
      setPreviewLine({ x1: lineStart.current.x, y1: lineStart.current.y, x2: pt.x, y2: pt.y });
    }
  };
  const onPointerUp = (e) => {
    if (dragId.current) { dragId.current = null; return; }
    if (lineStart.current) {
      const pt = toPercent(e.clientX, e.clientY);
      const dist = Math.hypot(pt.x - lineStart.current.x, pt.y - lineStart.current.y);
      if (dist > 3) setDiagram((p) => ({ ...p, lines: [...p.lines, { id: uid(), type: lineType, color: lineColor, x1: lineStart.current.x, y1: lineStart.current.y, x2: pt.x, y2: pt.y }] }));
      lineStart.current = null;
      setPreviewLine(null);
    }
  };
  const onCanvasClick = () => { if (!lineMode) setSelectedId(null); };

  const onElPointerDown = (id, e) => { e.stopPropagation(); dragId.current = id; setSelectedId(id); };
  ...
  const resizeZona = (delta) => setDiagram((p) => ({ ...p, elements: p.elements.map((el) => el.id === selectedId ? { ...el, size: Math.max(20, Math.min(160, (el.size || 45) + delta)) } : el) }));
  ...
  const fullSvg = useMemo(() => {
    let svg = buildDiagramSVG(diagram);
    if (previewLine) {
      const px_ = (x) => (x / 100) * DIAGRAM_W, py_ = (y) => (y / 100) * DIAGRAM_H;
      svg = svg.replace("</svg>", `<line x1="${px_(previewLine.x1)}" y1="${py_(previewLine.y1)}" x2="${px_(previewLine.x2)}" y2="${py_(previewLine.y2)}" stroke="${lineColor}" stroke-width="1.6" stroke-dasharray="4,3" opacity="0.8"/></svg>`);
    }
    return svg;
  }, [diagram, previewLine, lineColor]);
    ...
    <Modal title="Tarea gráfica" onClose={onClose} wide>
      <div className="flex flex-col gap-3">
        <div className="flex gap-1.5 flex-wrap">
          ...
          <Btn variant="ghost" onClick={undoLast}>↩ Deshacer</Btn>
        </div>

        <div className="relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: `${DIAGRAM_W}/${DIAGRAM_H}`, border: `2px solid ${T.line}` }}>
          <div ref={containerRef} onPointerDown={onCanvasPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onClick={onCanvasClick}
          ...
          <div className="absolute inset-0" style={{ pointerEvents: "none" }}>
            {diagram.elements.map((el) => (
              <div key={el.id} onPointerDown={(e) => onElPointerDown(el.id, e)}
                className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full"
                style={{ left: `${el.x}%`, top: `${el.y}%`, width: 30, height: 30, pointerEvents: lineMode ? "none" : "auto", cursor: "grab", border: selectedId === el.id ? `2px dashed ${T.red}` : "none" }} />
            ))}
          </div>
        </div>
          ...
          <div className="p-2.5 rounded-lg flex items-center gap-2 flex-wrap" style={{ background: "#EEF2F8" }}>
            ...
            {selectedDef?.isZona && (
              <>
                ...
                <Btn variant="ghost" onClick={() => resizeZona(-10)}>−</Btn>
                <Btn variant="ghost" onClick={() => resizeZona(10)}>+</Btn>
              </>
            )}
        ...
        {lineMode && (
          <div className="p-2.5 rounded-lg" style={{ background: "#FEF3C7", border: `1.5px solid ${T.yellow}` }}>
            <div className="text-xs font-bold mb-1.5" style={{ color: T.navy }}>Arrastrá sobre el campo, de inicio a fin, para trazar la línea</div>
        ...
        <Btn variant={lineMode ? "accent" : "ghost"} onClick={() => { setLineMode((v) => !v); lineStart.current = null; setPreviewLine(null); }}>{lineMode ? "Cerrar modo línea" : "✏️ Dibujar línea"}</Btn>
        ...
        {textPromptType && (
          <TextEntryModal title={textPromptType === "letra" ? "Letra a mostrar" : "Texto a mostrar"}
            onClose={() => setTextPromptType(null)}
            onSave={(val) => { addElement(textPromptType, val); setTextPromptType(null); }} />
        )}
      </div>
