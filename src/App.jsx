  ...
  return (
    <Modal title="Tarea gráfica" onClose={onClose} wide>
      <div className="flex flex-col gap-3">
        ...
        <div className="relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: `${DIAGRAM_W}/${DIAGRAM_H}`, border: `2px solid ${T.line}` }}>
          <div ref={containerRef} onClick={onCanvasClick}
            className="absolute inset-0 touch-none select-none" style={{ background: "#eee", cursor: lineMode ? "crosshair" : "default" }}
...
function buildPizarraSVG(state, players) {
  ...
  const elements = (state.elements || []).map((el) => {
    ...
    const color = el.team === "B" ? "#E63946" : "#1E3A8A";
...
function PizarraModule({ ctx }) {
  ...
  const usedIds = state.elements.filter((e) => e.type === "jugador").map((e) => e.playerId);
  ...
  const onCanvasPointerDown = (e) => {
    ...
    currentStroke.current = { id: uid(), points: [pt] };
  };
  const onCanvasPointerMove = (e) => {
    if (!isDrawingStroke.current) return;
    const pt = toPercent(e.clientX, e.clientY);
    currentStroke.current.points.push(pt);
    set((p) => ({ ...p, strokes: [...p.strokes.filter((s) => s.id !== currentStroke.current.id), { ...currentStroke.current }] }));
  };
  const onCanvasPointerUp = () => { isDrawingStroke.current = false; currentStroke.current = null; };
  ...
  const onCanvasClick = (e) => {
    ...
    if (!pendingPoint) setPendingPoint(pt);
      ...
      set((p) => ({ ...p, lines: [...p.lines, { id: uid(), type: lineType, color: "#1E3A8A", x1: pendingPoint.x, y1: pendingPoint.y, x2: pt.x, y2: pt.y }] }));
      setPendingPoint(null);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className="relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: `${DIAGRAM_W}/${DIAGRAM_H}`, border: `2px solid ${T.line}`, minHeight: "60vh" }}>
        <div ref={containerRef} onClick={onCanvasClick} onPointerDown={onCanvasPointerDown} onPointerMove={onCanvasPointerMove} onPointerUp={onCanvasPointerUp}
          className="absolute inset-0 touch-none select-none" style={{ background: "#eee", cursor: mode === "linea" ? "crosshair" : mode === "lapiz" ? "crosshair" : "default" }}
          dangerouslySetInnerHTML={{ __html: svgMarkup }} />
        <div className="absolute inset-0" style={{ pointerEvents: "none" }}>
      ...
      {selectedId && (
        <div className="p-2.5 rounded-lg flex items-center gap-2" style={{ background: "#EEF2F8" }}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Btn variant="ghost" onClick={() => setShowPicker("B")}>+ Jugador equipo B</Btn>
        <Btn variant="ghost" onClick={addBall}>+ Balón</Btn>
      </div>

      {showPicker && (
        <Modal title={`Elegir jugador — Equipo ${showPicker}`} onClose={() => setShowPicker(null)}>
          <div className="flex flex-col gap-1.5 max-h-[55vh] overflow-y-auto">
            {players.map((p) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Field label="Representación gráfica de la tarea" className="sm:col-span-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs" style={{ color: T.muted }}>o subí una imagen ya hecha abajo</span>
          </div>
